    ...
    public static void main(String[] args) throws SQLException {
        ...
        Javalin.create(config -> {
            ...
            config.routes.get("/usuarios", ctx -> {
              ...
              config.routes.get("/requerimentos", ctx -> {
                // crio um map <chave, valor> para que seja usado la no html
                Map<String, Object> map = new HashMap<>();
                // defino um apelido para a colecao de objetos de curso vindos do banco
                map.put("vetRequerimento", new RequerimentoDAO().listar());
                // renderizo a pagina html encaminhando tb o map
            ...
            config.routes.get("/requerimento/tela_alterar/{id}", ctx -> {
               ...
               config.routes.post("/requerimento/alterar", ctx -> {
                ...
                byte[] anexo;
                if (ctx.uploadedFile("anexo").content().readAllBytes().length > 0) {
                    anexo = ctx.uploadedFile("anexo").content().readAllBytes();
                } else {
                    anexo = null;
                }
                ...
                requerimento.setId(Integer.parseInt(ctx.formParam("id")));
                // baseado no tipo
                ...
                new RequerimentoDAO().alterar(requerimento);
                ctx.redirect("/requerimentos");
            });
